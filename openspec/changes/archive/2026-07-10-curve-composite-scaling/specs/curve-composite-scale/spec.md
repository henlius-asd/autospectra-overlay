...
生成 Y 轴范围时，系统 SHALL 基于复合缩放后的数据（`y * composite + scaleOffset`）计算 `rawDataMin` 和 `rawDataMax`，使 Y 轴范围容纳缩放后的曲线，避免 `clip: true` 裁剪。

#### Scenario: 全局放大后轴范围扩展

- **WHEN** 用户将 `globalScale` 调至 2.0
- **THEN** Y 轴范围上限扩展至能容纳缩放后的最大曲线值

#### Scenario: 手动缩小后轴范围收缩

- **WHEN** 用户将某条曲线的 `curveScales` 调至 0.1
- **THEN** Y 轴范围仍能正确显示所有曲线（不因该曲线压缩而异常）
