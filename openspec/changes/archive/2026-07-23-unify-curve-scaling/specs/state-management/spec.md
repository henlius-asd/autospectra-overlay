...
系统 SHALL 提供"新建工作区"功能。用户点击后经确认弹窗，SHALL 保留原始曲线数据（`curves`），清空所有可变状态——曲线移出叠图区（`visibleCurves`/`stagingOrder` 清空）、对齐偏移（`offsets`）、缩放（`curveScales`/`curveScaleOffsets`/`globalScale`）、层间距（`layerSpacing`）、标注（`braces`/`pointLabels`）、锁定（`locked`），并重置图表视图状态与 undo 历史。用户显示偏好 SHALL 保留。IndexedDB 工作区数据 SHALL 被清除。
